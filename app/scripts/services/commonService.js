...
commonModuleService.factory('commonService',
    function ($http, $state, $q, $uibModal,$cookies) {


       return { // our factory definition
        ...
        getBaseParams: function () {
          return {
            ...
            'url' : 'http://leshenghuo.jslsh.cn/interface/',
            'project': 'owWeb'
          };
        },
        ...
        //添加电话日志参数
        setCallLogs: {mobile: '', type: '', telBelong: '', city_id: '', page: '', business_id: '', ticket: ''},

        //添加广告日志
        addAdLog: function (aDObject) {
          //广告参数
          var logs = {
            ad_id: aDObject.id,
            city_id: sessionStorage.gpscityId,
            page: aDObject.param.page,
            ticket: $cookies.get('ticket')
          }
          var url = this.getBaseParams().url + 'addAdLog.action';
          this.getSyncData(url, logs).then();
        },
        //添加电话日志
        addCallLog: function (params) {
          var url = this.getBaseParams().url + 'addTelPhoneLog.action';
          this.getSyncData(url, params).then();
        }
