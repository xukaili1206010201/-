...
societyController.controller('societyCtrl', ['$scope', '$state', 'societyService', 'commonService', '$cookies', '$rootScope', function ($scope, $state, societyService, commonService, $cookies, $rootScope) {
  ...
  $scope.goTribe = function (buluoId) {
    //添加来源标志,方便在部落中点击返回时判断是由广告进入还是邻里社交进入
    localStorage.forumDetailFrom = '%society%';
...
//社交部落页
societyController.controller('tribeCtrl', ['$scope', '$state', 'societyService', '$stateParams', 'commonService', '$rootScope', '$location', '$cookies', '$q', 'userService','guanjiaService', function ($scope, $state, societyService, $stateParams, commonService, $rootScope, $location, $cookies, $q, userService,guanjiaService) {
  ...
  promise.then(function (data) {
    ...
    $scope.goPostDetail = function (bbsId) {
      localStorage.bbsDetailFrom = '%bbsList%';
    ...
    $scope.backSociety = function () {
      if(localStorage.forumDetailFrom =='%society%'){
        $state.go('society-Neighborhood');
      }
      if(localStorage.forumDetailFrom =='%guanggao%'){
...
societyController.controller('postCtrl', ['$scope', '$state', 'societyService', '$stateParams', 'commonService', '$cookies', '$location', function ($scope, $state, societyService, $stateParams, commonService, $cookies, $location) {
        ...
        case '0':
          ...
          $state.go('society-tribe', {
            'buluoId': $scope.bbsList.buluo_id
              ...
              break;
        case '-1':
          console.log("参数为空，删除失败！");
              break;
        case '-2':
          console.log("参数错误，删除失败！");
              break;
        case '-3':
  ...
  $scope.backTribe = function () {
    if (localStorage.bbsDetailFrom == '%bbsList%'){
      $state.go('society-tribe', {
        'buluoId': $scope.bbsList.buluo_id
      });
    }
    if (localStorage.bbsDetailFrom == '%guanggao%'){
      $state.go(localStorage.adRoute);
    }
...
societyController.controller('EditCtrl', ['$scope', '$state', 'societyService', '$stateParams', 'commonService', '$cookies', function ($scope, $state, societyService, $stateParams, commonService, $cookies) {
  ...
  $scope.submitBbsList = function () {
      if($scope.bbsList){
        if(!$scope.bbsList.title){
          commonService.showWarnMessage('请输入标题！');
          return;
        }
        if(!$scope.bbsList.bbs_content){
          commonService.showWarnMessage('请输入内容！');
          return;
        }
      }else{
          commonService.showWarnMessage('请输入标题！');
    ...
    societyService.editBBS($scope.param).success(function (response) {
      switch (response.code) {
        ...
        case '-2':
          ...
          break;
        case '-8':
          commonService.showErrorMessage("请先登录！");
          break;
      }
...
//帖子回复页
societyController.controller('replyCtrl', ['$scope', 'commonService','$state', 'societyService', '$stateParams', '$cookies', function ($scope,commonService, $state, societyService, $stateParams, $cookies) {
  /************************************ 初始化  ****************************************/
