...
//*********************************** 事件监听 ********************************
      //进入个人信息设置
      $scope.jumpEditUserInfo = function () {
        $state.go('member-personal-editUserInfo')
      };
...
personalModuleController.controller(
    'editUserInfoCtrl',
    function ($scope, $filter, $rootScope, $stateParams, userService, $state, commonService, $cookies) {
//*********************************** 初始化 *********************************
      $scope.formData = {};
      //获取用户信息
      userService.getUserInfo({
        ...
        switch (response.code) {
          case '0':
            ...
            var imgW = $(window).width() * 0.26;
            $('.inputBox').css('height', imgW + 'px');
            var ordImg = "<img id='ordImg' style='height:" + imgW + "px' src="
                + $scope.formData.headImgurl + ">";
            $('.imgBox #ordImg').css('height', imgW + 'px');
...
//*********************************** 事件监听 ********************************
      ...
      $scope.birthday = function () {
        ...
        if ($.trim(input) != '') {
          ...
          $scope.formData.birthday = year + "年" + month + "月" + date + '日';
        }
      };

      //身份证和生日绑定
      $scope.birthdayBlur = function () {
        var card = $scope.formData.id_number.toString();
        ...
        reader.onload = function (e) {
          ...
          img.onload = function () {
            if (lastFile.type == 'image/gif') {//gif格式的图片，不做任何处理
              ...
              $scope.base64 = canvas.toDataURL(lastFile.type);
            }
      ...
      $scope.jumpTo = function () {
        ...
        if ($.trim(card) != '') {
          var reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
          if (reg.test(card) === false) {
            commonService.showWarnMessage('身份证格式不正确！');
            return false;
          }
        }

        if (!$rootScope.regxIdnumber.test($scope.formData.id_number.toString()) && $scope.formData.id_number != '') {
          ...
          userService.editUserInfo(
            ...
            switch ($scope.result.code) {
              case '0':
                ...
                commonService.showSuccessMessage('保存成功！');
                break;
              case '-1':
                console.log("参数为空！");
                break;
