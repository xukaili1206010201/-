          ...
          switch (response.code) {
            case '0':
              ...
              if ($scope.cityList.length > 0) {
                $scope.errMsg = '';
              } else if ($scope.cityList.length == 0) {
                $scope.errMsg = '没有相关数据';
              }
              //数据请求完毕后重新进行排序
...
communityModuleController.controller(
    ...
    function ($scope, $stateParams, userService, $state, commonService, $cookies, $rootScope, reserveCacheService) {
      ...
      if (operate == 1) {
        ...
        //直接从本地存储的城市选择
        cityName = sessionStorage.gpscityName;
        cityId = sessionStorage.gpscityId;
      }

      $scope.currentSelectedCityName = cityName;
      $scope.apiParams = {
        'city_id': cityId,
        'key': null
      };
          ...
          switch (response.code) {
            case '0':
              ...
              if ($scope.communityList.length == 0) {
                $scope.errMsg = '没有相关数据！';
              } else if ($scope.communityList.length > 0) {
                $scope.errMsg = '';
              }
              //数据请求完毕后重新进行排序
              $.getScript('scripts/common/selectcity/sort.js', function () {
...
communityModuleController.controller(
    ...
    function ($rootScope, $scope, $stateParams, userService, $state, commonService, $cookies) {
      ...
      $scope.submitCommunityInfo = function () {
        ...
        if (userService.validateMobile($scope.telphone).validate == false) {
          commonService.showWarnMessage(userService.validateMobile($scope.telphone).msg);
          return;
        }
