...
cartModuleController.controller('cartCtrl', ['$scope', '$state', '$stateParams', 'userService', 'commonService', '$q', '$cookies', '$location', '$rootScope', '$timeout', 'payCacheService', function ($scope, $state, $stateParams, userService, commonService, $q, $cookies, $location, $rootScope, $timeout, payCacheService) {
  ...
  if (localStorage.skip != null) {
    ...
    $scope.cartParams.type = $scope.btnType == '预约' ? '家庭服务' : '特供服务,商家福利';
  }
  else {
    $scope.btnType = '购买';
    $scope.cartParams.type = '特供服务,商家福利';
  }
    ...
    userService.postRequestWithUrlAndParams("delCart.action", $scope.params).success(function (response) {
      switch (response.code) {
        ...
        case '-2':
          ...
          break;
        case '-3':
          commonService.showWarnMessage(response.data.data);
          break;
      }
    }).error(function (data, status, header, config) {
...
//减少商品的数量后重新计算商品的数量
  ...
  $scope.goGoodsDetail = function (goods) {
    localStorage.goodsId = goods.goods.goods_id;
    localStorage.goodsDetailFrom = "%cartList%";
    localStorage.skip = $stateParams.from + ':' + $scope.btnType + ':' + goods.cart_id;
    localStorage.goodsDetailFromId = goods.cart_id;//从订单或者购物车进商品详情然后进购买的时候需要传入对应的id
    $cookies.put('businessId', goods.goods.business_id);
    if ($scope.btnType == '购买')
