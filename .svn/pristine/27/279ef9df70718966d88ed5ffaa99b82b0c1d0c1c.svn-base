...
activitiesModuleController.controller('reserveCtrl',
    ['$scope', 'userService', '$state', 'commonService', 'payCacheService', '$timeout', '$interval', '$q', '$stateParams', '$cookies',
      function ($scope, userService, $state, commonService, payCacheService, $timeout, $interval, $q, $stateParams, $cookies) {
        ...
        $scope.changeStatus = function (status, elem) {
          ...
          $scope.keyValue = '';
          $scope.status = status;
          $scope.reserveParams.direction = 'down';
        ...
        //返回
        $scope.back = function () {
          if ($scope.reserveParams.key != '') {
            $scope.keyValue = '';
            $scope.reserveParams.key = '';
            $scope.searchReserve('');
            return;
          }
          else if (localStorage.getItem('goBack') == 'comservice') {
        ...
        $scope.goGoodsDetail = function (goods) {
          localStorage.goodsId = goods.goods_id;
          localStorage.goodsDetailFrom = "%reserveList%";
          localStorage.goodsDetailFromId = goods.orderItem_id;
          $state.go('guanjia-familyservicedetail', {goodsId: goods.goods_id});
        };
        /************************************ end ***************************************/
      }]);
